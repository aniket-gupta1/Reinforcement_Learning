...
class TRPO(object):
	...
	def sample(self, episode):
		...
		total_reward = sum(rs)
		return np.asarray(obs), np.asarray(Gs), np.asarray(total_reward), np.asarray(actions), np.asarray(action_probs)
	...
	def train_step(self,episode, obs, Gs, actions, action_probs, total_reward, t0):
		...
		Vs = self.value_net(np.atleast_2d(obs).astype('float32')).numpy().flatten()
		advantage = Gs - Vs 
