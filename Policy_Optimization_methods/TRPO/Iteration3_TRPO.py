...
class TRPO(object):
	...
	def __init__(self, env_name, env, policy_net, value_net = None,lam = 0.97, value_lr = 0.01, gamma = 0.99, delta = 0.1, 
	...
	def train_step(self,episode, obs, Gs, actions, action_probs, total_reward, entropy, advantage, t0):
		...
		with tf.GradientTape() as tape:
			logits = tf.nn.log_softmax(self.model(np.atleast_2d(np.array(obs)).astype('float32')))
			random_var = tf.squeeze(tf.one_hot(np.array(actions), self.env.action_space.n))
			log_probs = tf.math.reduce_sum(logits * random_var, axis=1)
			policy_loss = tf.math.reduce_mean(advantage * log_probs)

		policy_gradient = np.asarray(tape.gradient(policy_loss, self.model.trainable_variables))
		flatten_policy_gradient = tf.concat([tf.reshape(g,[-1]) for g in policy_gradient], axis = 0).numpy()
		#policy_gradient = flatgrad(surrogate_loss, self.model.trainable_variables).numpy()
		# policy_loss2 = surrogate_loss()
		# policy_gradient2 = flatgrad(surrogate_loss, self.model.trainable_variables).numpy()
		# step_direction2 = conjugate_grad(hessian_vector_product, policy_gradient2)
		#print("difference:  ",np.sum(flatten_policy_gradient - policy_gradient2))

		step_direction = conjugate_grad(hessian_vector_product, flatten_policy_gradient)
