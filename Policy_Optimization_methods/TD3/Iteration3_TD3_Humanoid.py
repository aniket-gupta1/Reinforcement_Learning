...
name = f'Humanoid-v2_Iteration3_{time_var.tm_mon}_{time_var.tm_mday}_{time_var.tm_hour}_{time_var.tm_min}_{time_var.tm_sec}'
...
class DDPG():
	...
	def __init__(self, env_name, env, Actor_net, Critic_net1, Critic_net2, act_dim, obs_dim ,lam = 0.97,decay = False,
				record_vid = False, Actor_lr = 0.0001, gamma = 0.99, delta = 0.01, Critic_lr = 0.001, render = False, epoch_steps = 2000, 
				value_train_iterations = 5, memory_size = 100000, polyak_const = 0.995, minibatch_size = 2048):
		...
		self.path = f"/home/aniket/Deep_RL/RL_practice/Policy_Gradient_Methods/TD3/{name}"
	...
	def add_noise(self, action, step, decay = False):
		"""Function to add OU noise to the action for exploration"""
		# if decay:
		# 	self.noise_scale *= 1/((step+1)**0.5)
	...
	def make_video(self, n=1, path = None, print_var = True):
		if path==None:
			path = self.path
	...
	def train_step(self, episode):
		...
		if self.record_vid and episode%200==0:
			print("Recording the data")
			self.make_video(n = 20, path = self.path + "/"+ str(episode), print_var = False)
	...
	def train(self, episodes):
		...
		for episode in range(episodes):
			...
			if episode%10 == 0 and episode != 0:
				self.Actor.save_weights(f"{name}/Episode{episode}/actor.ckpt")
				self.Critic1.save_weights(f"{name}/Episode{episode}/critic1.ckpt")
				self.Critic2.save_weights(f"{name}/Episode{episode}/critic2.ckpt")
...
if __name__ == "__main__":
	...
	agent = DDPG(env_name, env, Actor_net, Critic_net1, Critic_net2, decay = True, render=False, record_vid = True ,act_dim = act_dim, obs_dim = obs_dim)
	episodes = int(sys.argv[2]) if sys.argv[2] else int(200)

	if sys.argv[3] == "test":
		path = str(sys.argv[4])
		agent.load_weights_test(path)
		agent.make_video(int(sys.argv[5]))
	else:
		if sys.argv[4] == 'load':
			actor_weights_path = input("Enter the actor weights path: ")
			critic1_weights_path = input("Enter the critic weights path: ")
			critic2_weights_path = input("Enter the critic weights path: ")

			agent.load_weights_train(actor_weights_path, critic1_weights_path, critic2_weights_path)
