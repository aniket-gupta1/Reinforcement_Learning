...
# Aim: TO implement a object oriented code for PPO here using this link
# https://github.com/spideralessio/TRPO-Tensorflow2/blob/master/TRPO.py

# WITH KL CONSTRAINT

# prob_ratio = 1 in Iteration 2, trying to resolve that here

# Initialize summary writer object to visualise data in the tensorboard
time_var = time.localtime()
name = f'MountainCar-v0 {time_var.tm_mon}_{time_var.tm_mday}_{time_var.tm_hour}_{time_var.tm_min}_{time_var.tm_sec}'
...
class TRPO(object):
	...
	def render_episode(self, n=1):
		for i in range(n):
			...
			while True:
				...
				next_state, reward, done, _ = self.envs[0].step(action)
				state = next_state
				t_r+=reward
				if done:
					print(temp)
	...
	def train_step(self,episode, obs, Gs, actions, action_probs, total_reward, t0):
		...
		if episode == 0:
			...
			model1 = self.tmp_model
			model1.load_weights(f"{name}/Episode{episode-1}.ckpt")
		with tf.GradientTape() as policy_tape:
			logits = self.model(np.atleast_2d(obs).astype('float32'))
			action_prob = tf.cast(tf.nn.log_softmax(logits), dtype='float32')
			action_prob = tf.reduce_sum(actions_one_hot * action_prob, axis=1)
			old_logits = model1(np.atleast_2d(obs).astype('float32'))
			old_action_prob = tf.cast(tf.nn.log_softmax(old_logits),dtype='float32')
		...
		for _ in range(50):
			...
			action_prob = tf.cast(tf.nn.log_softmax(logits), dtype='float32')
			action_prob = tf.reduce_sum(actions_one_hot * action_prob, axis=1)
			kl = tf.reduce_mean(old_action_prob - action_prob)
			if kl>0.01:
				break
...
if __name__ == "__main__":
	...
	policy_model = Model((len(env.observation_space.sample()),), env.action_space.n, 'tanh', 'linear', [64,64])
	value_net = Model((len(env.observation_space.sample()),), 1, 'tanh', 'linear', [64,64])
