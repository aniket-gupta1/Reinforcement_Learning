...
name = f'Pendulum-v0 {time_var.tm_mon}_{time_var.tm_mday}_{time_var.tm_hour}_{time_var.tm_min}_{time_var.tm_sec}'
	...
	def train_step(self, episode):
		...
		for t in itertools.count():
			...
			if self.memory.can_provide_sample(self.minibatch_size):
				...
				states, actions, rewards, next_states, dones = np.asarray(batch[0]),np.asarray(batch[1]),\
											np.asarray(batch[2]),np.asarray(batch[3]),np.asarray(batch[4])

				with tf.GradientTape() as critic_tape:
					# Represent the actions suggested by the target Actor network
					target_actions = self.Target_Actor(np.atleast_2d(next_states).astype('float32'))*self.act_limit
					# Represent the target Q values for the target actions by the target Critic network
					target_Q_values = self.Target_Critic(np.atleast_2d(tf.concat([next_states, target_actions], axis = -1)).astype('float32'))
					# Bellman backup
		...
		print(f"Ep:{episode} total_reward:{total_reward:0.2f} actor_loss:{np.mean(actor_losses):0.2f} critic_loss:{np.mean(critic_losses):0.2f}")	
...
if __name__ == "__main__":
	...
	agent = DDPG(env_name, env, Actor_net, Critic_net, render=False, act_dim = act_dim, obs_dim = obs_dim)
